...
If there are three qualities of models that cannot all be maximized---realism, precision, and generality [@costanzaModelingComplexEcological1993a]---our point is to maximize realism and generality. And if increasing resolution to accurately describe the underlying processes means decreasing predictability [@costanzaModelingComplexEcological1993a], our goal is to explore and describe an urban system comprised of climatic, social, and environmental subsystems that interact in complex ways, moving towards a universal methodology. Assessing the variability of public health as it relates to the urban system can allow for prioritization of resources to the most vulnerable communities. This is largely uncharted territory, both technically and pragmatically, but a better understanding of the interactions between these systems can the information necessary to increase environmental justice and decrease health disparities. 
...
which states that the probability of a parameter given the data is proportional to the probability of the data conditioned on the parameter multiplied by the marginal probability for the parameter. In other words, the posterior distribution is the the prior distribution weighted by the observed data. Consider there are $G$ groups of observations:
...
where $Q$ is the distribution of the observations within the groups each having its own parameter $\theta_g$ 
...
and $W$ is the  distribution of the group parameters $\theta_g$ defined by parameter $\gamma$. $\gamma$ is called the hyperparameter and also has its own distribution. The hyperparameter also has its own distribution. The posterior distribution for all the unknown parameters is:
...
where $p(\theta|\gamma)p(\gamma)$ is the marginal joint distribution for the two parameters. The posterior distribution for the hyperparameter $\gamma$ that governs the group-level parameters is given by its marginal distribution:
