...
\noindent and $W$ is the  distribution of the group parameters $\theta_g$ defined by parameter $\gamma$. $\gamma$ is called the hyperparameter and also has its own distribution. The hyperparameter also has its own distribution. The posterior distribution for all the unknown parameters is:
...
\noindent where $p(\theta|\gamma)p(\gamma)$ is the marginal joint distribution for the two parameters. The posterior distribution for the hyperparameter $\gamma$ that governs the group-level parameters is given by its marginal distribution:
...
The primary objective of this research is to construct a statistically robust model that integrates climatic, environmental, and social systems. To accomplish this, every step must be thoroughly considered and made with informed intent. Designing the architecture of the model will be challenging and time intensive and model specification will be contingent upon extensive research into the specifics of Bayesian hierarchical models as well as the structures of the component subsystems. This modeling framework will inform our understanding of the dominant covariance structures and collinearities in the urban system that give rise to variations in pediatric asthma. 
