...
The successful model of these coupled systems can be parsed in many ways and the model results can be used to explain the processes that lead to the variation in pediatric asthma in more detail. Even if the framework is not successful, the failure can guide other attempts. Regardless, the methodology should provide enough information to prioritize variables in subsequent models whether they are general and conceptual or instantiated and precise. Constructing a modeling framework that incorporates climatic, environmental, and social systems at several spatial and temporal scales may provide a methodology suitable to much more than pediatric asthma in the Kansas City metro area. Moving forward, the pursuit is of an understanding of how the urban system emerges from the interactions of its constituent subsystems.

# Methods {#methods}

The first step in the proposed research will be to construct a database of the desired variables (See \@ref(data)) for the duration of the available asthma data. Presumably, all data will be aggregated to the census tract level per day. Processing of the remotely sensed data will be done using Google Earth Engine with all other variables being processed in R. The dimensionality of the variable space may be quite high, in which case, reduction techniques such as principal component analysis (PCA) will be explored. For instance, @brewerDoesNeighborhoodSocial2017 used PCA to construct a concentrated disadvantage index from a combination of socio-economic variables. The next step will be a thorough review of the relevant literature. One of the key tasks of Bayesian analysis is the assignment of priors and while it is always possible to assign an uninformative prior with a diffuse distrubtion, if there is substantive knowledge the parameters can be constrained into finite regions, improving model estimation [@gelmanBayesianDataAnalysis2013]. If there are known mechanistic relationships between variables, this would guide the assingment of priors. Additionally, all variables and their relationships with asthma counts as well as the other covariates will be explored descriptively. The hierarchy of the data and groups within each level need to be specified. Model specification is contingent upon extensive research on the intricacies of implementing Bayesian hierarchical models.

Model checking and sensitivity analysis will be conducted after the posterior distributions of the parameters and hyperparamters are estimated in order to improve the model. It is likely that several models will be constructed both for comparison in terms of selected variables and structured hierarchies, and through iterative improvement. It will hopefully be apparent which variables are key representations of their respective subsystems. These variables and their causal role in the variation of pediatric asthma can be explored in more depth with an eye towards the possibility of effective intervention.

A key difficulty and opportunity of this research will be the effective visualization of the high-dimensional output. Novel ways of mapping how the parameters change in time and space will be an important contribution of this research. Possibilities include animations and interactive maps. Interactive maps will allow the exploration of how the variation of pediatric asthma could change with the manipulation of variables.
