...
$$P(Y_i=y_i|x_i,z_i)=
\left.\begin{cases}
\psi_i+(1-\psi_i)e^{-\mu_i} & \text{if } y_i=0 \\
(1-\psi_i)\frac{e^{-\mu_i}\mu_i^{y_i}}{y_i!} & \text{if } y_i > 0 \\
\end{cases}\right\}$$
where $\mu_i$ is the conditional mean and $\psi_i$ is the probability that observation $i$ is characterized by the zero-count process, written as 
$$\psi_i=F(z_i'\gamma) $$
where $z_i$ is a vector of covariates and $\gamma$ is the vector of coefficients of a logit or probit model. 

Another option is to standardize the counts by the number of children, calculating the effective asthma rate per spatial area per day. The rate is still zero inflated but accounts for the differing number of children per area and transforms the count into a continuous dependent variable. __This clearly needs to be expanded upon.__

Many researchers acknowledge the necessity of a hierarchical approach to modeling public health data [@brewerDoesNeighborhoodSocial2017; @Liu2007a; @Uejio2011; @williamsSocialDeterminantsTaking2009; @bestComparisonBayesianSpatial2005]. Individuals are nested within neighborhoods which are nested in urban areas. The levels of the hierarchy are similar in their spatial and temporal scales and interact with higher and lower levels in systematic ways, where the higher levels act as constraints and the lower levels as noise [@costanzaModelingComplexEcological1993a]. Failure to model the non-linear variability in the lower levels can lead to significant aggregation error [@costanzaModelingComplexEcological1993a]. Furthermore, hierarchical models simplify complicated systems by separating joint distributions of covariates into separate conditional models which are linked by probability rules with uncertainty quantified at every level [@wikleHierarchicalModelsEnvironmental2003]. In particular, Bayesian spatio-temporal models account for the spatial autocorrelation in the data through spatial random effects, temporal effects account for the time-dependency of the data, time-varying covariates model the uncertainty attributable to measured confounders, and space-time interaction effects model the additional variation unaccounted for by the random effects [@Khana2018]. These models can borrow strength both spatial and temporally [@Khana2018] 
...
\theta_g\sim{W(\gamma)}
$$
where $W$ is the common distribution of $\theta_g$ with parameter $\gamma$, also known as the hyperparameter also with its own distribution. Taken together the posterior distribution for all the unknown parameters is:
$$
p(\gamma, \theta|y) \propto p(y|\theta, \gamma)p(\theta|\gamma)p(\gamma)
$$
where $p(\theta|\gamma)p(\gamma)$ is the marginal joint distribution for the two parameters. The posterior distribution for the hyperparameter $\gamma$ is given by its marginal distribution:
$$
